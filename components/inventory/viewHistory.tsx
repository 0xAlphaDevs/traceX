        ...
        history.map((item, index) => (
          <React.Fragment key={index}>
            ...
            {index !== history.length - 1 && (
              <span>
                <Link className="font-bold text-slate-500 h-10 w-10" />
              </span>
            )}
          </React.Fragment>
        ))}
...
async function verifyHistory(attestationId: string) {
  ...
  try {
    const attestation: any = await getAttestation(attestationId);
    ...
    if (!attestation) {
      ...
      if (!attestation.data.previousAttestationId) {
        // console.log("no linked attestation id");
        return chainOfAttestations;
      } else {
        let linkedAttestationId = attestation.data.previousAttestationId;
        while (linkedAttestationId !== "") {
          const previousAttestation: any = await getAttestation(
            linkedAttestationId
          );
          ...
          if (previousAttestation) {
            ...
            linkedAttestationId = previousAttestation.data.previousAttestationId
              ? previousAttestation.data.previousAttestationId
              : "";
          }
        }
      }
